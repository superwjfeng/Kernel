...
struct task_struct {
	...
	unsigned int			flags; // flags 是进程号,在调用fork()时给出
	unsigned int			ptrace;
	...
	unsigned int			policy;
	int				nr_cpus_allowed;
	const cpumask_t			*cpus_ptr; // 允许进程在哪一个CPU上运行
	cpumask_t			cpus_mask;
	...
	/*这两个指针指向内存描述符
	对于普通进程，mm和active_mm指向同一个内存描述符
	对于内核线程，mm是空指针，active_mm从进程借用内存描述符
	*/
	struct mm_struct		*mm;
	...
	const struct cred __rcu		*real_cred; // 此成员指向主体相应课题的证书
	 ...
	 */
	char				comm[TASK_COMM_LEN]; // 进程名称，进程正在运行的可执行文件名
